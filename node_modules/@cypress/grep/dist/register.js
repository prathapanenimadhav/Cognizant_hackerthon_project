...
function register() {
    ...
    let grep = Cypress.expose('grep');
    if (grep) {
        grep = String(grep).trim();
    }
    const grepTags = Cypress.expose('grepTags') || Cypress.expose('grep-tags');
    const burnSpecified = Cypress.expose('grepBurn') || Cypress.expose('grep-burn') || Cypress.expose('burn');
    const grepUntagged = Cypress.expose('grepUntagged') || Cypress.expose('grep-untagged');
    if (!grep && !grepTags && !burnSpecified && !grepUntagged) {
        debugInstance('Nothing to grep, version %s', package_json_1.version);
        return;
    }
    const grepBurn = Cypress.expose('grepBurn') ||
        Cypress.expose('grep-burn') ||
        Cypress.expose('burn') ||
        1;
    const omitFiltered = Cypress.expose('grepOmitFiltered') || Cypress.expose('grep-omit-filtered');
